...
load_wikibase_register <- function(dir = "../register", dry_run = TRUE,
  ...
  if (!dry_run && nrow(out) > 0) {
    write_wikibase_pages(session, list(certificates = wikibase_certificates_wikitext(
      out, records$certificates, read_register_wikidata(dir)
    )), summary = "generated by codecheck::load_wikibase_register()", log_file = log_file)
  }
...
#' @param register_qids the certificates' Wikidata items as `register.csv`
#'   records them, see [read_register_wikidata()]
...
wikibase_certificates_wikitext <- function(written, certificates,
  ...
  # The item the export created on Wikidata, which only the register records.
  wikidata_cell <- function(qid) {
    if (length(qid) == 0 || is.na(qid)) "&mdash;" else paste0("[https://www.wikidata.org/wiki/", qid, " ", qid, "]")
  }
  ...
  rows <- lapply(seq_len(nrow(items)), function(i) {
    ...
    c(
      ...
      wikidata_cell(unname(register_qids[text(source$`Certificate ID`)]))
    )
  })
  ...
  c(
    ...
    "! Certificate !! Checked paper !! Title !! Venue !! Check date !! Register !! Report DOI !! Wikidata",
