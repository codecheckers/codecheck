...
#' The pages the bootstrap writes
#'
#' Everything that depends on the model and the instance's numbering, but not
#' on the register's data.
#'
#' @keywords internal
BOOTSTRAP_WIKIBASE_PAGES <- c("data_model", "example_queries", "about", "copyrights",
                              "copyright_redirect", "main")

#' Fill in the local ids of the entities a plan finds present
#'
#' @param plan a plan from [plan_wikibase_entities()]
#' @param existing the instance's entities, from [wikibase_mapping()]
#' @return the plan with a `local_id` column, `NA` for what is still to create
#' @keywords internal
wikibase_plan_local_ids <- function(plan, existing) {
...
bootstrap_wikibase <- function(dry_run = TRUE, log_file = NULL) {
  ...
  plan <- wikibase_plan_local_ids(plan_wikibase_entities(existing), existing)
  ...
  if (dry_run) {
    ...
    for (page in BOOTSTRAP_WIKIBASE_PAGES) {
      cli::cli_alert_info("Would write {.url {wikibase_page_url(WIKIBASE_PAGES[[page]]$title)}}")
    }
  ...
  # Written on every run, not only when something was created: the data model
  # page is the index of the instance, and an unchanged instance still deserves
  # a current one. The queries are written from the same plan, so the local ids
  # in them are the ids the entities were just created under. The hosting-policy
  # pages and the Main Page are here so that an instance rebuilt from empty
  # comes back compliant without anybody remembering to write them by hand.
  context <- new.env(parent = emptyenv())
  context$plan <- plan
  write_wikibase_pages(
    session,
    lapply(stats::setNames(BOOTSTRAP_WIKIBASE_PAGES, BOOTSTRAP_WIKIBASE_PAGES),
           wikibase_page_wikitext, context = context),
