...
wikibase_examples_wikitext <- function(plan, generated_at = Sys.time()) {
  ...
  c(
    ...
    paste0("* '''The numbers are this instance's own.''' A Wikibase mints its own ",
           ...
           "its counterpart; [[", WIKIBASE_PAGES$data_model$title, "]] is the table of both."),
    ...
    paste0("The model these queries are written against is described on [[",
           WIKIBASE_PAGES$data_model$title, "]], and the certificates themselves are listed on [[",
           WIKIBASE_PAGES$certificates$title, "]]. The register remains the authority: ",
