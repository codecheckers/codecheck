...
* The "all persons" table (`docs/persons/index.html`) now carries the per-venue-type stacked bar for checks conducted (register#92), the same visualisation `create_all_codecheckers_table()` already had before that table was retired in favour of `docs/persons/index.html`. `create_all_persons_table()` computes it from the person's codechecker-role rows only, mirroring the codecheckers table's logic; `index.json` gets the plain counts under `"Check types"`, the rendered page gets the bar.
