...
* The data model for representing CODECHECK certificates as linked data, in the CODECHECK Wikibase and in Wikidata (register#50). `R/wikidata.R` is deliberately only the model plus its accessors - no HTTP, no rendering, no QuickStatements - so that the modelling decisions sit in one reviewable place rather than spread over the code that will act on them. New `wikidata_model()`, `wikidata_entity_kinds()`, `wikidata_statements()`, `wikidata_properties()`, `wikidata_endpoint()`, `wikidata_creates()` and `validate_wikidata_model()`.
  Two decisions in it are not obvious. Every certificate carries `P13046` *publication type of scholarly work*: the query service is split into two graphs, an entity lives in exactly one of them, and a cross-graph reference resolves to a single marker triple with no label, type or identifier - so without `P13046` a certificate lands in the main graph and no query can join it to the paper it checks. Membership follows the *direct* `P31` value against a fixed list of QIDs or the presence of `P13046`; the `P279` hierarchy is not evaluated, which is why a certificate that is transitively a scholarly article still ends up in the main graph. And the link to the checked paper is `P6977` *review of*, not `P2860` *cites work*: `P6977` is equivalent to `schema.org/review`, which is what a certificate page already publishes as a `Review` with an `itemReviewed`, while `P2860` cannot express which of several cited works was the one checked. On certificate items `P6977` is used for nothing but the checked paper, and `validate_wikidata_model()` fails a model that reintroduces `P2860` there.
  Only certificates are created on Wikidata; the papers, people and venues they refer to are resolved against existing items and created only in the CODECHECK Wikibase, which mirrors everything.
  The certificate identifier is carried generically as `P528` *catalog code*, qualified with `P972` *catalog*, until a dedicated "CODECHECK certificate ID" external identifier property is proposed and created - a catalog code names nothing without saying which catalog it belongs to, and the validator rejects a `P528` statement without that qualifier. The catalog item for the register does not exist yet, so the statement is marked pending and no export emits it; new `wikidata_pending()` lists what a complete export is still waiting on.
  Both venues are modelled, on the two items they belong to: `P1433` on the certificate names the platform the certificate itself is published on (Zenodo, OSF or ResearchEquals, derived from the report DOI the way `detect_report_platform()` already does it), while `P1433` on the checked paper names the venue the article appeared in. The paper's venue is resolved from the paper's own record (its publication ISSN), not from the register's `Venue` column: a register venue names a conference or journal as an institution, which is not the same thing as the publication it used - AGILE papers appear in *AGILE: GIScience Series* (Q126264390, ISSN 2700-8150) today, while earlier years of the same conference were published in Springer LNCS (Q924044), so a venue-derived value would be wrong for the older ones. Because both are publication series with an ISSN, they are work-class items and valid `P1433` values, and no event property (`P5072`) is needed. Papers with no publication ISSN - preprints, reports - get no statement.
  A certificate's `P31` is chosen per venue: the AGILE reproducibility reviews are typed as Q116740071 *Reproducibility Report* rather than Q116740091 *CODECHECK*, since they follow the same practice but are not branded as CODECHECKs.
...
* New `/persons/<ORCID>/` landing page (register#123), replacing `/codecheckers/`: covers both roles a person can have - paper author and codechecker - showing "Works authored" and "Checks conducted" as two separate tables (confirmed against a mockup on real register data), with role counts in `stats.json` and a `Person` + `Review`/`ScholarlyArticle` Schema.org graph. Only ORCID-identified people get a page, matching #123's explicit decision not to attempt name matching. New `add_person_records()`, `explode_person_records()`, `generate_person_metadata_html()`, `generate_person_schema_org()`, `create_all_persons_table()`. `/codecheckers/` is removed by a full render (its own `register.json`/`register.csv` files are no longer generated); a `docs/404.html` (new `generate_404_page()`) redirects a stray `/codecheckers/<X>/` link to `/persons/<X>/` and, for an unmatched `/works/<DOI>`, explains that no CODECHECK has been registered for that DOI rather than showing a generic error.
...
* `register.md` is no longer generated for a person page, since its two tables (works authored, checks conducted) can't be represented as the one markdown table every other filter's `register.md` assumes; its HTML footer accordingly shows no "Markdown" link. New `CONFIG$FILTERS_WITHOUT_MD`.
* The "CSV source"/"CSV on GitHub" footer links (formerly "searchable CSV") now show only on the main, unfiltered register page - every filtered detail page's own CSV was a small subset of the same data already in the table above it.
