...
- Fediverse accounts and venue hashtags (register#217): the codechecker
  lists and the register’s `persons.csv` have a `fediverse` column
  (`@user@instance`), and `venues.csv` has `fediverse` and `hashtags`
  (`;`-separated, without `#`). Person and venue pages show the account
  as a `rel="me"` link, their Schema.org metadata list it under
  `sameAs`, and the `index.json` of a person, codechecker or venue
  carries it; venue pages and JSON also list the hashtags. `persons.csv`
  accounts win over the codechecker lists, and a value that is not
  `@user@instance` is ignored rather than linked.
